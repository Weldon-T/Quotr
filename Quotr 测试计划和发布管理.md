# Quotr 每周 Release QA 计划 v2.0

**日期**：2026-08-03
**环境**：test.quotr.ai
**基于**：对测试站点的完整产品探索（登录、4 个模块、API 层、数据状态）
...
> **注意**：此列表基于对测试站点的浏览器网络流量捕获。仅包含探索过程中实际触发的 API 调用。创建/更新/删除类端点（如 create-project、update-supplier、delete-template）尚未被观察到，但大概率存在。入职后需向开发获取完整 API 文档或 Swagger/OpenAPI schema，补充缺失端点。
...
| 场景 | 方法 | 优先级 |
|------|------|--------|
| 同项目并发编辑 | 两个用户同时编辑同一项目估价 → 验证后保存者收到冲突提示或被正确合并 | P1 |
| 供应商并发修改 | 用户 A 编辑供应商信息时用户 B 删除该供应商 → 验证 A 收到明确错误提示 | P1 |
| 项目创建并发 | 两个用户同时创建同名项目 → 验证唯一性约束工作正常 | P2 |
...
- **多账号需求**：IDOR 和并发测试需要至少 2 个不同组织的测试账号。入职后向管理员申请
...
> **版本**：v2.0 | **日期**：2026-08-03 | **下次审阅**：季度或产品重大变更时
