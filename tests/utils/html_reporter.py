...
BUG_KB = {
    "test_csp_header_present": {
        "id": "BUG-001", "title": "CSP header 缺失 / Missing Content-Security-Policy Header", "severity": "P0", "module": "Security",
        "description": "HTTP 响应缺少 CSP header，站点无 XSS 防护。Quotr 将 JWT 存储于 localStorage，XSS 可导致 token 泄露。\nHTTP responses lack a CSP header; the site has no XSS protection. Quotr stores JWT in localStorage, so XSS can leak tokens.",
        "fix": "在 Cloudflare/CDN 层添加 Content-Security-Policy 响应头 / Add Content-Security-Policy header at CDN layer.",
    },
    "test_hsts_header_present": {
        "id": "BUG-001", "title": "HSTS header 缺失 / Missing Strict-Transport-Security Header", "severity": "P0", "module": "Security",
        "description": "缺少 HSTS header，无法强制 HTTPS，存在中间人攻击风险。\nMissing HSTS header; HTTPS cannot be enforced, creating a MITM risk.",
        "fix": "添加 Strict-Transport-Security: max-age=31536000; includeSubDomains",
    },
    "test_content_type_options_header_present": {
        "id": "BUG-001", "title": "X-Content-Type-Options header 缺失 / Missing X-Content-Type-Options Header", "severity": "P0", "module": "Security",
        "description": "缺少 nosniff 指令，浏览器可能进行 MIME 类型嗅探。\nMissing nosniff directive; browsers may perform MIME type sniffing.",
        "fix": "添加 X-Content-Type-Options: nosniff",
    },
    "test_frame_options_header_present": {
        "id": "BUG-001", "title": "X-Frame-Options header 缺失 / Missing X-Frame-Options Header", "severity": "P0", "module": "Security",
        "description": "站点可被嵌入 iframe，存在 Clickjacking 攻击风险。\nSite can be embedded in an iframe, creating a Clickjacking risk.",
        "fix": "添加 X-Frame-Options: DENY 或 SAMEORIGIN",
    },
    "test_dashboard_redirects_to_login": {
        "id": "BUG-002", "title": "Dashboard 不强制重定向未认证用户 / Dashboard Does Not Redirect Unauthenticated Users", "severity": "P0", "module": "Auth",
        "description": "未登录状态下访问 /dashboard/project，页面不重定向到登录页。可能是反爬改动的副作用。\nVisiting /dashboard/project while unauthenticated stays on the page instead of redirecting to login. Possibly a side effect of anti-bot changes.",
        "fix": "检查 React Router auth guard 逻辑 / Check React Router auth guard logic.",
    },
    "test_query_org_healthy": {
        "id": "BUG-003", "title": "Dashboard 白屏或 query-org 异常 / Dashboard White Screen / query-org Failure", "severity": "P0", "module": "Dashboard",
        "description": "/api/query-org 是 Dashboard 加载的第一个 API，失败会导致整个 Dashboard 白屏。\n/api/query-org is the first API the Dashboard loads; failure causes a full white screen.",
        "fix": "优先排查 query-org 返回值和 SPA 渲染逻辑 / Investigate query-org response and SPA rendering logic.",
    },
    "test_database_bug_not_regressed": {
        "id": "BUG-004", "title": "Database 模块主内容区空白 / Database Module Main Content Area Blank", "severity": "P0", "module": "Database",
        "description": "/dashboard/database 仅渲染侧边栏，主内容区无任何 DOM 元素。无 console error。\n/dashboard/database renders only the sidebar; the main content area has no DOM elements. No console errors.",
        "fix": "检查 React Router 中 Database 路由注册和 Suspense/lazy 配置 / Check Database route registration and Suspense/lazy config.",
    },
    "test_dashboard_template_a11y": {
        "id": "BUG-006", "title": "Template 页面 A11y 违规 / Template Page A11y Violations (button-name, color-contrast)", "severity": "P1", "module": "A11y",
        "description": "Template 页面存在 serious 级 WCAG 违规：按钮缺少可访问名称、颜色对比度不足。\nTemplate page has serious WCAG violations: buttons missing accessible names, insufficient color contrast.",
        "fix": "为图标按钮添加 aria-label，调整颜色满足 4.5:1 对比度 / Add aria-label to icon buttons, adjust colors to meet 4.5:1 contrast.",
    },
    "test_dashboard_suppliers_a11y": {
        "id": "BUG-007", "title": "Suppliers 页面 A11y 违规 / Suppliers Page A11y Violations (aria-command-name, button-name, color-contrast)", "severity": "P1", "module": "A11y",
        "description": "Suppliers 页面存在 serious 级 WCAG 违规：ARIA 命令缺少名称、按钮缺少可访问名称、颜色对比度不足。\nSuppliers page has serious WCAG violations: ARIA commands missing names, buttons missing accessible names, insufficient color contrast.",
        "fix": "为 ARIA command 和图标按钮添加 aria-label，调整对比度 / Add aria-label to ARIA commands and icon buttons, adjust contrast.",
    },
}
...
<title>Quotr 测试报告 / Test Report — {datetime.now().strftime('%Y-%m-%d %H:%M')}</title>
<style>
...
<body>
<div class="container">
<h1>Quotr 测试报告 / Test Report</h1>
<p class="subtitle">环境 / Env：test.quotr.ai | {datetime.now().strftime('%Y-%m-%d %H:%M')} | Playwright + Pytest | 模式 / Mode：{'CI 分层累积 / Layered' if layers[0]['name'] != '—' else '完整单次 / Full Suite'}</p>

<h2>执行摘要 / Execution Summary</h2>
<div class="grid grid5">
<div class="card"><div class="stat-value">{self.total}</div><div class="stat-label">总用例 / Total</div></div>
<div class="card"><div class="stat-value pass">{self.passed}</div><div class="stat-label">通过 / Passed</div></div>
<div class="card"><div class="stat-value fail">{self.failed}</div><div class="stat-label">失败 / Failed</div></div>
<div class="card"><div class="stat-value skip">{self.skipped}</div><div class="stat-label">跳过 / Skipped</div></div>
<div class="card"><div class="stat-value error">{self.errors}</div><div class="stat-label">错误 / Errors</div></div>
</div>
<div class="grid grid5" style="margin-top:16px">
<div class="card"><div class="stat-value pass">{self.pass_rate():.0f}%</div><div class="stat-label">通过率 / Pass Rate</div></div>
<div class="card"><div class="stat-value">{len(self._bug_ids)}</div><div class="stat-label">产品 Bug / Product Bugs</div></div>
<div class="card"><div class="stat-value blocked">{self.blocked}</div><div class="stat-label">阻塞 / Blocked (Env)</div></div>
<div class="card"><div class="stat-value">{len(stats)}</div><div class="stat-label">模块 / Modules</div></div>
<div class="card"><div class="stat-value">{datetime.now().strftime('%H:%M')}</div><div class="stat-label">生成时间 / Generated</div></div>
</div>

<h2>模块覆盖 / Module Coverage</h2>
<table>
<tr><th>模块 / Module</th><th>用例 / Tests</th><th>通过 / Pass</th><th>失败 / Fail</th><th>跳过 / Skip</th><th>错误 / Error</th><th>通过率 / Rate</th></tr>
{self._module_rows(stats)}
</table>

<h2>发现的 Bug / Bugs Found{'（本次未触发 / None triggered this run）' if not self.bugs else ''}</h2>
{self._bug_cards()}

<h2>全部用例详情 / All Test Details</h2>
<details><summary>查看全部 / View all {self.total} 个用例 / tests</summary><div class="content"><table>
<tr><th>层级 / Layer</th><th>模块 / Module</th><th>用例 / Test</th><th>结果 / Result</th><th>耗时 / Duration</th></tr>
{self._all_rows()}
</table></div></details>
...
<footer>Quotr Test Report — Auto-generated — {datetime.now().isoformat()}</footer>
    ...
    def _module_rows(self, stats: list) -> str:
        ...
        return "\n".join(rows) if rows else "<tr><td colspan='7'>无数据 / No data</td></tr>"
    ...
    def _all_rows(self) -> str:
        ...
        return "\n".join(rows) if rows else "<tr><td colspan='5'>无数据 / No data</td></tr>"

    def _bug_cards(self) -> str:
        if not self.bugs:
            return "<p style='color:var(--pass)'>本次运行未触发已知产品 Bug。/ No known product bugs triggered in this run.</p>"
        cards = []
        for bid, info in self.bugs.items():
            ...
            cards.append(
                ...
                f'<div class="meta"><span>模块 / Module：{info["module"]}</span><span>ID：{info["id"]}</span></div>'
                f'<p>{info["description"]}</p><p><strong>修复 / Fix：</strong>{info["fix"]}</p></div>'
            )
        return "\n".join(cards)
